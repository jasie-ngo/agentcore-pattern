...
| [0002](0002-dual-agent-over-single-agent.md) | Dual-Agent Over Single-Agent | Eliminates confirmation bias, since a single agent rarely overrides its own first decision |
...
| [0012](0012-gsi-over-scan-for-status-queries.md) | GSI Over Scan for Status Queries | Eliminates full table scans, using DynamoDB GSIs for O(results) instead of O(table) |
...
| [0015](0015-config-driven-agent-fabric-orchestration.md) | Config-Driven Agent Fabric for Orchestration | Design-stage decisions log for reimagining the pipeline as a declarative, config-driven agent fabric supporting multi-client, resumable, tenant-isolated workflows |
