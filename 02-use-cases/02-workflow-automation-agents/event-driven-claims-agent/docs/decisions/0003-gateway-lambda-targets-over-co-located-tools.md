...
1. **Cedar Policy Engine requires a Gateway.** Cedar policies evaluate before each tool call. Without a Gateway, there's no policy enforcement point, so the agent could call any tool unconditionally. This sample's $100k blocking policy is the primary Cedar demonstration.
