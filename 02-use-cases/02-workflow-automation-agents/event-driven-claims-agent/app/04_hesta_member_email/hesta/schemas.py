...
Verbatim port of ``app/hesta-claimsagent/models.py``.

Every specialist agent returns one of these validated Pydantic objects.  The
YAML names the class it wants — ``agent_kwargs.structured_output: IntentResult``
— and ``hesta/factory.py`` resolves the name against this module and passes the
class to ``Agent(structured_output_model=...)``.  When such an agent is invoked
as a delegate tool, strands hands the coordinator the validated JSON rather
than free text, which is what keeps the orchestration contracts intact.

The deterministic tools (``assess_attachments``, ``classify_verification``,
``route_decision``, ``filter_member_cases``) return the same shapes as dicts,
so the whole pipeline speaks one vocabulary.
"""
...
class DetectedIntent(BaseModel):
    ...
    intent_id: str = Field(
        description="A taxonomy intent id (see intents/taxonomy.py) or 'other_unknown'."
    )
    ...
    evidence_quote: str = Field(
        default="", description="A short verbatim span from the email supporting it."
    )
...
class IntentResult(BaseModel):
    ...
    intents: list[DetectedIntent] = Field(
        description="All intents found, highest confidence first."
    )
...
class CaseSummary(BaseModel):
    ...
    outstanding_items: list[str] = Field(
        default_factory=list, description="What is still needed to progress."
    )
...
class MemberProfile(BaseModel):
    ...
    member_number: str | None = Field(
        default=None, description="Resolved member/policy number, if any."
    )
    matched: bool = Field(default=False, description="A DynamoDB record was found for the number.")
    match_key: str | None = Field(
        default=None, description="How the record was found: member_number | none."
    )
    factors_matched: list[str] = Field(
        default_factory=list,
        description="Which identifiers matched: member_number, email, account_type, status.",
    )
    account_type: str | None = Field(
        default=None, description="Account/policy type from the record."
    )
    member_status: str | None = Field(default=None, description="active | expired | closed …")
    verification_level: str = Field(
        default="unverified", description="verified | partial | unverified"
    )
    verification_required: bool = Field(
        default=True, description="True until identity is sufficiently verified."
    )
    notes: str = Field(
        default="", description="Human-readable explanation of the verification outcome."
    )
...
class AttachmentAssessment(BaseModel):
    ...
    expected_document: str = Field(
        default="none", description="Document expected for the primary intent, or 'none'."
    )
    status: str = Field(
        default="not_applicable", description="ok | missing | present_unverified | not_applicable"
    )
...
class CaseStatusResult(BaseModel):
    ...
    member_pending: list[dict] = Field(
        default_factory=list,
        description="Pending cases whose policy/member number matches this member.",
    )
...
class EmpathyAssessment(BaseModel):
    ...
    complaint_indicator: bool = Field(
        default=False, description="True if the email reads as a complaint."
    )
    priority: str = Field(description="low | normal | high | urgent")
    recommended_attention: str = Field(
        description="One line on how the human agent should approach this member."
    )
...
class DraftEmail(BaseModel):
    ...
    verification_state: str = Field(
        description="verified | needs_verification — drives what the draft asks for."
    )
    kb_snippets_used: list[str] = Field(
        default_factory=list, description="Which inline HESTA snippets informed it."
    )
    assumptions: list[str] = Field(
        default_factory=list, description="Assumptions a human should confirm."
    )
...
class ReviewResult(BaseModel):
    ...
    approved_for_human_send: bool = Field(
        description="True if the draft is safe for a human to send as-is."
    )
    accuracy_ok: bool = Field(
        description="Facts/claims are consistent with the request and HESTA knowledge."
    )
    tone_ok: bool = Field(description="Tone matches HESTA's supportive house style.")
    compliance_ok: bool = Field(
        description="No unverified regulated action, no promises, correct next steps."
    )
    edits: str = Field(default="", description="Suggested edits, or '' if none.")
    issues: list[str] = Field(
        default_factory=list, description="Specific problems found, or empty."
    )
...
class RoutingDecision(BaseModel):
    ...
    escalate_to_human: bool = Field(
        description="True if a human must review before anything is sent."
    )
    reasons: list[str] = Field(default_factory=list, description="Why the case was escalated.")
    regulated: bool = Field(
        default=False, description="True if the primary intent is a regulated request."
    )
