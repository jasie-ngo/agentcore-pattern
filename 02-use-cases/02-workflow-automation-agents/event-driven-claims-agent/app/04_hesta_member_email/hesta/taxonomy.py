"""The 8 HESTA member-request intents + signals — the pipeline's shared taxonomy.

Verbatim port of ``app/hesta-claimsagent/intents/taxonomy.py``.

``id`` is the machine value the pipeline uses and ``code`` is the ground-truth
label.  ``regulated`` drives the human-in-the-loop gate (``route_decision``) and
``expected_attachment`` feeds AI-004's expectation check (``assess_attachments``).

``render_for_prompt()`` produced the taxonomy block that is inlined verbatim in
``agents.intent_identifier.system_prompt`` in ``config.yaml`` — run it after
editing ``INTENTS`` and paste the output back into the YAML.
"""
