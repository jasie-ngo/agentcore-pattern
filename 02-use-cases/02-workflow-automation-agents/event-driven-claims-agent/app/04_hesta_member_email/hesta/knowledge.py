...
Verbatim port of ``app/hesta-claimsagent/knowledge/hesta_snippets.py``.
...
reaches it through the ``hesta_knowledge`` and ``identity_verification_block`` tools
(see ``hesta/tools.py``), which ``config.yaml`` wires onto the writer agent.
Nothing here promises a regulated outcome.
"""
